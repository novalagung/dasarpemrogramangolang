...
Time merupakan topik yang penting dalam pemrograman. Di bab ini kita akan belajar tentang pemanfaatan time, property-property yang disediakan, dan juga *format* & *parsing* data `string` ke `time.Time` dan sebaliknya. 

Time disini maksudnya adalah gabungan *date* dan juga *time*, bukan hanya waktu saja.
...
`time.Time` adalah tipe data objek time. Cara untuk membuat data bertipe ini ada dua, yaitu:
...
## Layout Format Untuk Keperluan Parsing Time

Golang juga menyediakan beberapa layout format umum yang bisa dimanfaatkan. Jadi tidak perlu menuliskan kombinasi komponen-komponen layout format di atas.

Salah satu layout format yang bisa digunakan adalah `time.RFC822`. Format ini sama dengan `02 Jan 06 15:04 MST`. Berikut adalah contoh penerapannya.
